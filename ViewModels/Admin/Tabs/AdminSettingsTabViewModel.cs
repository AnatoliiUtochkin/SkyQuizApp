...
{
    public class AdminSettingsTabViewModel : INotifyPropertyChanged
    {
        ...
        public string CreatedAtFormatted =>
            _session.CurrentUser?.CreatedAt.ToString("dd.MM.yyyy HH:mm") ?? string.Empty;

        public ICommand ChangeEmailCommand { get; }

        public AdminSettingsTabViewModel(AppDbContext context, IUserSessionService session)
        {
            _context = context;
            _session = session;
        ...
        private void ChangeEmail()
        {
            ...
            _logger.Log(LogAction.EmailChange, $"Email змінено з {oldEmail} на {Email}", user.UserID);
        ...
        private void UpdateTwoFactorSetting()
        {
            ...
            var action = _isTwoFactorEnabled ? LogAction.TwoFactorEnable : LogAction.TwoFactorDisable;
            string message = _isTwoFactorEnabled
                ? "Двофакторну автентифікацію увімкнено."
                : "Двофакторну автентифікацію вимкнено.";
            _logger.Log(action, message, user.UserID);
        }
